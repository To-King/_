...
async def getToken():
    msg = await;
Client.get_message('@pang_da_bot')
    await msg[0].click(0)
    msgR = await
Client.get_messages('@pang_da_bot', limit=1)
    msgR = msgR[0]
    token = re.search(r"你的Token (.*)\n"),msgR.message).group(1)
    return token

def replaceToken(token):
    with open(configFile, 'r') as f:
         txt = f.read()
         txt = txt.replace(re.search(r'PANDA_TOKEN="(.*)"',txt).group(1), f'{token}')
         txt = txt.replace(re.search(r'Panda_token="(.*)"',txt).group(1), f'{token}')
...
async def renew():
    msg = await
Client.get_message('@pang_da_bot')
    await msg[0].click(data=b'keep_alive')
Client.get_message('@pang_da_bot', limit=1)
    msgR = msgR[0]
    expDate = re.search(r'(有效期:.*)', msg.message).group(1)
    return expDate */
