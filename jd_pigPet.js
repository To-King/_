...
!(async () => {
  ...
  if (process.env.PIGPETSHARECODE) {
    ...
    let res = await getAuthorShareCode('https://raw.githubusercontent.com/ugg999999/Profiles/master/shareCodes/pigPet.json')
    if (!res) {
      res = await getAuthorShareCode('https://raw.fastgit.org/ugg999999/Profiles/master/shareCodes/pigPet.json')
    }
    if (res){
  ...
  console.log(`\n======开始大转盘助力======\n`);
  $.helpId = await getAuthorShareCode('https://raw.fastgit.org/ugg999999/Profiles/master/shareCodes/pigPet.json');
              ...
              if (data.resultData.resultCode === 0 && n < 5) {
                ...
                for (let i = 0; i < $.friends.length; i++) {
                  if ($.friends[i].status === 1) {
                    ...
                    if (!['ugg999999'].includes($.name)) { //放过孩子吧TT
