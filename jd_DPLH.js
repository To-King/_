...
//if (!DAPAINEW){
 //   console.log(`\n请填写大牌联合通用开卡的活动ID,变量是DAPAINEW\n`)
//    return;
//}
//let actId = process.env.DPLHTY || '';    //是否清空，如果为false，则上面设置了多少就只删除多少条
//let actId = process.env.DAPAINEW || '';    //是否清空，如果为false，则上面设置了多少就只删除多少条 //新加的
!(async () => {
    if (!cookiesArr[0]) {
...
async function openCardNew() {
    $.token = null;
    $.buyerNick = 'tAgCbJ04JVEZn0MSGTVgi8jNhNaYFy2HteErE6izlhTf9nrGY7gBkCdGU4C6z/xD';//6s
    $.activityInfo = null;
    await getToken();
    if ($.token) {
