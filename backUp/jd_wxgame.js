...
!(async () => {
    ...
    if (!wxgameActivityId) {
        $.log(`没有通用ID任务，尝试获取远程`);
        let data = await getData("https://raw.githubusercontent.com/ugg999999/Profiles/master/shareCodes/wxgame.json")
        if (!data) {
            data = await getData("https://raw.githubusercontent.com/ugg999999/Profiles/master/shareCodes/wxgame.json")
        }
        if (data && data.length) {
            $.log(`获取到远程且有数据`);
